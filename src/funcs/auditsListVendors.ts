 ...
 */
export async function auditsListVendors(
  client: LoganVantaCore,
  request: operations.ListVendorsRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.PaginatedResponseVendor,
    | APIError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  ...
  const path = pathToFunc("/audits/{auditId}/vendors")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.PaginatedResponseVendor$inboundSchema),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
    return result;
